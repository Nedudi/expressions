(function($) {
  ...
  if(!$.fn.getBounds) {
    $.fn.getBounds = function(transformObject, refObject) {
      ...
      for(var qq = 0; qq < this.length; qq++) {
        ...
        if(!dd || dd == "") dd = $(transformObject).css("OTtransform");
        if(!dd || dd == "") dd = $(transformObject).css("msTransform");
        if(!dd || dd == "") dd = $(transformObject).css("MozTransform");
        if(!dd || dd == "") dd = $(transformObject).css("WebkitTransform");
        if(dd) trData = dd.match(/matrix\([0-9\.\,\s\+\-]+\)/);
        if(trData) {
          if(trData[0]) trData = trData[0];
          if(trData) trData = trData.substr(7,dd.length - 8);
          if(trData) trData = trData.split(",");
          if(trData) {
            wdt = Math.abs(ww*trData[0]) + Math.abs(hh*trData[1]);
            hgt = Math.abs(ww*trData[2]) + Math.abs(hh*trData[3]);
          } else {
            wdt = ww;
            hgt = hh;
          }
          ...
          if(trData && trData[1]) {
            wdt = Math.abs(ww*Math.cos(trData[1])) + Math.abs(hh*Math.sin(trData[1]));
            hgt = Math.abs(ww*Math.sin(trData[1])) + Math.abs(hh*Math.cos(trData[1]));
          } else {
            wdt = ww;
            hgt = hh;
          }
  ...
  jQuery.browser = {
    ...
    detect: function() {
      ...
      this.mobile = !!(this.iPhone || this.iPod || this.iPad || this.Android || mobile || this.isTouchDevice);
    }
  };
