    ...
    init: function(options) {
      return this.each(function() {
        ...
        that.updateFontSize = function(){
          ...
          that.view.p.css('font-size',100*1.3+'%');
        };
        ...
        that.checkMaxLength = function(e){
          if(that.nonFormattedText.length > that.options.maxLength) {
            ...
            if($.inArray(e.keyCode, keys) === -1 && !(e.shiftKey || e.ctrlKey || e.metaKey)) {
              e.preventDefault();
              e.stopPropagation();
            }
          }
        ...
        that.view.e.on('keyup keypress', function(e){
        ...
        that.view.e.on('focus', function() {
          if(!that.view.e.text()){
            if(that.options.msie || that.options.ff) that.view.e.html('<br/>'); // I cnow.. feature detection and etc.. but this is not the case
          }
        });
